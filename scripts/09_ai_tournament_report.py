"""
Script 09 — AI Tournament Intelligence Report
===============================================
Purpose: Use the Claude API (Anthropic) to synthesise all computed metrics into
         a structured, analyst-grade tournament intelligence report — delivered
         in natural language with strategic depth.

This script is the data engineering pipeline's final stage:
  Raw events → Engineered metrics (scripts 01-08) → AI narrative synthesis

Why this matters for data engineering:
  Computing metrics is step one. Making them *intelligible and actionable* is
  step two. This script demonstrates the full pipeline: structured tabular data
  in, expert-level natural language insight out — a pattern applicable to any
  analytics domain (finance, operations, logistics, healthcare).

What Claude receives:
  - xG per player table (finishing quality)
  - PPDA per team table (pressing efficiency)
  - Press success rate per team (conversion quality)
  - Match state pressing table (tactical adaptability)
  - xG conceded vs pressing table (defensive outcome linkage)

What Claude produces:
  - A multi-section markdown report covering:
    1. Tournament executive summary
    2. Attacking talent analysis (xG model findings)
    3. Pressing philosophy rankings (PPDA + success rate synthesis)
    4. Tactical adaptability insights (match state behaviour)
    5. The central research finding (pressing → shot quality tradeoff)
    6. A data-driven "best XI" based purely on metrics
...
Run: python scripts/09_ai_tournament_report.py
"""
...
# ── Load all computed metric tables ──────────────────────────────────────────
...
# ── Format tables as markdown for the prompt ──────────────────────────────────
def to_md(df: pd.DataFrame, n: int = 20) -> str:
    return df.head(n).to_markdown(index=False, floatfmt=".2f")

# ── Build the prompt ──────────────────────────────────────────────────────────
# We give Claude precise, structured data and ask for specific analytical sections.
# Prompt caching is applied to the large data block (it stays constant across calls).
